...
This is the one piece of Task 3 that genuinely requires two humans (Ishan and Charvee Saraiya) —
...
2. **Both of you independently**, without looking at `audit_key.csv` or discussing scores
   ...
   `audit_sheet_template.csv` (copy it to `rater_ishan.csv` and `rater_charvee_saraiya.csv`) with a
...
3. Once both `rater_ishan.csv` and `rater_charvee_saraiya.csv` are complete, compute agreement:
   ```bash
...
CSVs (`rater_ishan.csv`, `rater_charvee_saraiya.csv`) and `audit_agreement_results.json` are **not
